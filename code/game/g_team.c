...
const char *TeamColorString(int team) {
	if (team==TEAM_RED)
		...
		return S_COLOR_BLUE;
	else if (team==TEAM_REDCOACH || team==TEAM_BLUECOACH)
...
//ssdemo
	...
	if ( ent == NULL ) {
		G_SendServerCommand( -1, va("print \"%s\"", msg) );
	}
	else {
...
void AddTeamScore(vec3_t origin, int team, int score) {
	...
	if ( team == TEAM_RED ) {
		if ( level.teamScores[ TEAM_RED ] + score == level.teamScores[ TEAM_BLUE ] ) {
			...
			te->s.eventParm = GTS_TEAMS_ARE_TIED_RED;
		}
		...
		if ( level.teamScores[ TEAM_BLUE ] + score == level.teamScores[ TEAM_RED ] ) {
			//teams are tied sound
			te->s.eventParm = GTS_TEAMS_ARE_TIED_BLUE;
		}
...
qboolean OnSameTeam( gentity_t *ent1, gentity_t *ent2 ) {
	...
	if ( ent1->client->sess.sessionTeam == TEAM_RED && ent2->client->sess.sessionTeam == TEAM_REDCOACH) {
		return qtrue;
	}
	if ( ent1->client->sess.sessionTeam == TEAM_REDCOACH && ent2->client->sess.sessionTeam == TEAM_RED) {
		return qtrue;
	}
	if ( ent1->client->sess.sessionTeam == TEAM_BLUE && ent2->client->sess.sessionTeam == TEAM_BLUECOACH) {
		return qtrue;
	}
	if ( ent1->client->sess.sessionTeam == TEAM_BLUECOACH && ent2->client->sess.sessionTeam == TEAM_BLUE) {
		return qtrue;
	}
...
void Team_SetFlagStatus( int team, flagStatus_t status ) {
	...
	if( modified ) {
		...
		G_SetConfigstring( CS_FLAGSTATUS, st ); //ssdemo
	}
}
...
Report a location closest to "origin". Uses placed nearby target_location entities
============
*/
gentity_t *Team_GetLocationFromOrigin(vec3_t origin, qboolean useitems) {
	gentity_t		*eloc, *best;
	float			bestlen, len;
	...
	if (useitems) {
		for (eloc = g_entities; eloc < &g_entities[MAX_GENTITIES]; eloc++) {
			if (!eloc->inuse || !eloc->item || (eloc->flags & FL_DROPPED_ITEM)) continue;

			if (eloc->item->giType == IT_POWERUP || eloc->item->giType == IT_WEAPON ||
					eloc->item->giType == IT_HOLDABLE || eloc->item->giType == IT_PERSISTANT_POWERUP ||
					eloc->item->giType == IT_TEAM ||
					(eloc->item->giType == IT_ARMOR && eloc->item->quantity >= 50) ||
					(eloc->item->giType == IT_HEALTH && eloc->item->quantity >= 100)) {
...
============
*/
gentity_t *Team_GetLocation(gentity_t *ent, qboolean useitems)
{
	vec3_t			origin;

//freeze
	if ( ent->freezeState ) {
		if ( is_body( ent->target_ent ) ) {
...
//freeze
	...
	return Team_GetLocationFromOrigin( origin, useitems );
}

qboolean Team_GetLocationMsgFromOrigin(vec3_t origin, char *loc, int loclen, qboolean useitems) {
	gentity_t *best;

	best = Team_GetLocationFromOrigin( origin, useitems );
	...
	if ( best->item ) {
		Com_sprintf(loc, loclen, "%s", best->item->pickup_name);
	}
	else {
		if (best->count) {
			if (best->count < 0) {
...
============
*/
qboolean Team_GetLocationMsg(gentity_t *ent, char *loc, int loclen, qboolean useitems)
{
	gentity_t *best;

	best = Team_GetLocation( ent, useitems );
...
//freeze
	if ( ent->client->sess.sessionTeam == TEAM_REDCOACH || ent->client->sess.sessionTeam == TEAM_BLUECOACH ) {
		Com_sprintf(loc, loclen, "%s", "Coach");
		return qtrue;
	}

	if ( best == ent ) {
		Com_sprintf(loc, loclen, "%s", "waiting to play");
		return qtrue;
	}
//freeze
...
gentity_t *SelectRandomTeamSpawnPoint( int teamstate, gentity_t *ent ) {
	gentity_t	*spot;
	int			count;
	int			selection;
...
============
*/
gentity_t *SelectCTFSpawnPoint ( gentity_t *ent, int teamstate, vec3_t origin, vec3_t angles ) {
	gentity_t	*spot;

	spot = SelectRandomTeamSpawnPoint ( teamstate, ent );
...
//freeze
//ssdemo
	static int	lastSentTime[TEAM_NUM_TEAMS];
//ssdemo
...
//freeze
			frozen = 0;
			unfreezer = ENTITYNUM_WORLD;

			if ( player->freezeState && player->target_ent ) {
				h = a = 0;
				if ( player->target_ent->count ) {
					frozen = player->target_ent->count - level.time;
					frozen = (int)(100.0f - (float)frozen / 1000.0f / g_thawTime.value * 100);
					if ( frozen < 0 ) {
						frozen = 0;
					}
					else if ( frozen > 100 ) {
					...
					if ( player->target_ent->unfreezer ) {
						unfreezer = player->target_ent->unfreezer->s.number;
					}
				}
				else {
					frozen = level.time - player->target_ent->timestamp;
					frozen = (int)(100.0f - (float)frozen / 1000.0f / g_autoThawTime.integer * 100);

					if ( player->target_ent->llamatime ) {
						int newfrozen = level.time - player->target_ent->llamatime;
						newfrozen = (int)(100.0f - (float)newfrozen / 1000.0f / g_llamaPenalty.value * 100);
...
//				level.sortedClients[i], player->client->pers.teamState.location, h, a, 
				...
				player->client->ps.weapon, player->s.powerups, frozen, unfreezer);
//freeze - added frozen to info string
			j = strlen(entry);
...
//ssdemo
	// only record one of these per frame
	if ( lastSentTime[ent->client->sess.sessionTeam] < level.time ) {
		// mark it
		lastSentTime[ent->client->sess.sessionTeam] = level.time;
		// record team instead of client number
		G_RecordServerCommand( ent->client->sess.sessionTeam, va("tinfo %i %s", cnt, string) );
	}
//ssdemo
}
	...
	if (level.time - level.lastTeamLocationTime > TEAM_LOCATION_UPDATE_TIME) {
		...
		for (i = 0; i < g_maxclients.integer; i++) {
			...
			if (ent->inuse && (ent->client->sess.sessionTeam == TEAM_RED ||	ent->client->sess.sessionTeam == TEAM_BLUE)) {
				loc = Team_GetLocation( ent, qfalse );
//freeze
				if (loc == ent) {
					ent->client->pers.teamState.location = -1;
				}
//freeze
				else if (loc)
