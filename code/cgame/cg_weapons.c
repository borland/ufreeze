...
//freeze
	if ( ci - cgs.clientinfo != cg.clientNum ) {
		le->endTime = cg.time + cg_otherRailTrailTime.value;
	} else {
//freeze
...
//pmskins
	if ( cg_teamRails.integer == 2 && cg.predictedPlayerState.stats[STAT_TEAM] != TEAM_SPECTATOR ) {
		vec3_t color;

		if ( ci->team == TEAM_FREE || ci->team != cg.predictedPlayerState.stats[STAT_TEAM] ) {
			VectorCopy( cgs.enemyRailColor, color );
		}
		else {
			VectorCopy( cgs.railColor, color );
		}
...
//pmskins
//freeze
	else if ( cg_teamRails.integer ) {
		vec3_t color;

		if ( ((ci->team == TEAM_BLUE || ci->team == TEAM_BLUECOACH) && !cg_swapSkins.integer) ||
				((ci->team == TEAM_RED || ci->team == TEAM_REDCOACH) && cg_swapSkins.integer) ) {
			// blue rail
			color[0] = 0.3f;
			color[1] = 0.3f;
			color[2] = 1.0f;
...
//nomip
		if ( cg_nomip.integer & NOMIP_LIGHTNING ) {
			...
			cgs.media.lightningExplosionModel = trap_R_RegisterModel( "models/weaphits/crackle_nomip.md3" );
		}
		else {
//nomip
...
//nomip
		if ( cg_nomip.integer & NOMIP_ROCKETEXPL )
...
//nomip
		if ( cg_nomip.integer & NOMIP_GRENADEEXPL )
...
//nomip
	...
	case WP_RAILGUN:
		...
		MAKERGB( weaponInfo->flashDlightColor, 0.8f, 0.5f, 1.0f );
...
//nomip
	...
	case WP_BFG:
		...
		MAKERGB( weaponInfo->flashDlightColor, 0.4f, 1.0f, 0.4f );
		weaponInfo->flashSound[0] = trap_S_RegisterSound( "sound/weapons/bfg/bfg_fire.wav", qfalse );
//nomip
		if ( cg_nomip.integer & NOMIP_BFG ) {
			cgs.media.bfgExplosionShader = trap_R_RegisterShader( "bfgExplosion_nomip" );
			weaponInfo->missileModel = trap_R_RegisterModel( "models/weaphits/bfg_nomip.md3" );
		}
		else {
//nomip
...
===============
*/
static void CG_LightningBolt( centity_t *cent, vec3_t origin1, vec3_t origin3 ) {
	trace_t  trace;
	refEntity_t  beam;
	vec3_t   forward;
...
//unlagged - true lightning
	...
	// use the nifty three-plane graphic type
	beam.reType = RT_LIGHTNING;
	// bolt shader
	beam.customShader = cgs.media.lightningShader;

	// use the provided origins, even though they may be slightly
	// different than the muzzle origin
//multiview
	// if they sent in an origin to be used for first-person views
	if ( origin1 != NULL ) {
		// add it
		VectorCopy( origin1, beam.origin );
		CG_AddRefEntityToScene( &beam, cent->currentState.clientNum, REF_FIRST_PERSON, qfalse );
	}

	// if they sent in an origin to be used for third-person views
	if ( origin3 != NULL ) {
		// add it
		VectorCopy( origin3, beam.origin );
		CG_AddRefEntityToScene( &beam, cent->currentState.clientNum, REF_THIRD_PERSON, qfalse );
	}
//multiview
	...
	if ( trace.fraction < 1.0 ) {
		...
		CG_AddRefEntityToScene( &beam, cent->currentState.clientNum, REF_ALWAYS, qfalse ); //multiview

//unlagged - attack prediction #2
//unlagged - LG bandwidth
		if ( cg.time - cent->lastLightningStrike >= 50 &&
				(cent->currentState.number != cg.predictedPlayerState.clientNum ||
				(cent->currentState.number == cg.clientNum && cgs.delagHitscan &&
					(cg_delag.integer & 1 || cg_delag.integer & 8))) ) {

			//CG_Printf("Predicted LG missile hit\n");

			// record this for checking later
			cent->lastLightningStrike = cg.time;

			// did it hit a player?
			if ( cg_entities[trace.entityNum].currentState.eType == ET_PLAYER ) {
				CG_MissileHitPlayer( WP_LIGHTNING, trace.endpos,
						trace.plane.normal, trace.entityNum, 0 );
			}
			else {
				CG_MissileHitWall( WP_LIGHTNING, cent->currentState.clientNum,
...
//ssdemo
	...
	if ( cent->pe.barrelSpinning == !(cent->currentState.eFlags & EF_FIRING) ) {
		cent->pe.barrelTime = time;
		cent->pe.barrelAngle = AngleMod( angle );
		cent->pe.barrelSpinning = !!(cent->currentState.eFlags & EF_FIRING);
#ifdef MISSIONPACK
		if ( cent->currentState.weapon == WP_CHAINGUN && !cent->pe.barrelSpinning ) {
			CG_StartSound( cent->lerpOrigin, cent->currentState.number, CHAN_WEAPON, trap_S_RegisterSound( "sound/weapons/vulcan/wvulwind.wav", qfalse ) );
		}
#endif
	}
...
========================
*/
static void CG_AddWeaponWithPowerups( refEntity_t *gun, centity_t *cent, int refFlags ) {
	entityState_t *state = &cent->currentState;
	// add powerup effects
	if ( (state->powerups & (1 << PW_INVIS)) &&
			!(state->powerups & (1 << PW_SIGHT)) &&
			!cg.predictedPlayerState.powerups[PW_SIGHT] ) {
		CG_AddInvisibleEntity( gun, cent, cgs.clientinfo[cent->currentState.clientNum].team, refFlags );
	} else {
		CG_AddRefEntityToScene( gun, cent->currentState.number, refFlags, qtrue );
...
//	int	col;
	...
	if ( !ps ) {
		...
		if ( ( cent->currentState.eFlags & EF_FIRING ) && weapon->firingSound && cg.time >= cg.timein ) {
	...
	CG_AddWeaponWithPowerups( &gun, cent, ps ? REF_ONLY_CLIENT | REF_NOT_PIP : REF_THIRD_PERSON ); //multiview
	...
	if ( weapon->barrelModel ) {
		...
		CG_AddWeaponWithPowerups( &barrel, cent, ps ? REF_ONLY_CLIENT | REF_NOT_PIP : REF_THIRD_PERSON ); //multiview
	}
...
//ssdemo
	...
	CG_AddRefEntityToScene( &flash, cent->currentState.clientNum, ps ? REF_ONLY_CLIENT | REF_NOT_PIP : REF_THIRD_PERSON, qtrue );
	...
	// add lightning bolt
	// if it's the view weapon
	if ( ps ) {
		// only send a first-person origin
		CG_LightningBolt( nonPredictedCent, flash.origin, NULL );
	}
	else {
		// it's not the view weapon

		// if the player is the predicted player (main view)
		if ( cent->currentState.number == cg.predictedPlayerState.clientNum ) {
			// this was called from CG_Player() - send only a third-person origin
			CG_LightningBolt( nonPredictedCent, NULL, flash.origin );
		}
		else {
			// it's a regular player called from CG_Player() - send two origins
			CG_LightningBolt( nonPredictedCent, cent->lerpOrigin, flash.origin );
		}
	}
}
...
void CG_AddViewWeapon( playerState_t *ps ) {
	...
	if ( ps->persistant[PERS_TEAM] == TEAM_SPECTATOR ||  ps->persistant[PERS_TEAM] == TEAM_REDCOACH ||  ps->persistant[PERS_TEAM] == TEAM_BLUECOACH) {
		return;
	}
	...
	if ( !cg_drawGun.integer ) {
		...
		if ( cg.predictedPlayerState.eFlags & EF_FIRING ) {
			...
			CG_LightningBolt( &cg_entities[ps->clientNum], origin, NULL );
		}
		return;
	}
...
//freeze
	cg_entities[cent->currentState.clientNum].lastFiringTime = cg.time;
//freeze
	...
	if ( cent->currentState.powerups & ( 1 << PW_QUAD ) ) {
		CG_StartSound( cent->lerpOrigin, ent->number, CHAN_ITEM, cgs.media.quadSound );
	}
...
*/
//freeze - added count
void CG_MissileHitPlayer( int weapon, vec3_t origin, vec3_t dir, int entityNum, int count ) {
//freeze
	if ( Q_IsFrozenBody( entityNum ) ) {
		int i;
		for ( i = 0; i < ( (count > 0) ? count : 1 ); i++ ) {
			CG_IceChip( origin, entityNum );
		}
	}
	else
//freeze
...
static void CG_ShotgunPellet( vec3_t start, vec3_t end, int skipNum ) {
	...
	if ( cg_entities[tr.entityNum].currentState.eType == ET_PLAYER ) {
		CG_MissileHitPlayer( WP_SHOTGUN, tr.endpos, tr.plane.normal, tr.entityNum, 1 ); //freeze - added count
	} else {
		if ( tr.surfaceFlags & SURF_NOIMPACT ) {
...
//	VectorMA( finish, cg_tracerWidth.value, right, verts[0].xyz );
	VectorAdd( finish, right, verts[0].xyz );
	verts[0].st[0] = 0;
	verts[0].st[1] = 1;
...
//	VectorMA( finish, -cg_tracerWidth.value, right, verts[1].xyz );
	VectorSubtract( finish, right, verts[1].xyz );
	verts[1].st[0] = 1;
	verts[1].st[1] = 0;
...
//	VectorMA( start, -cg_tracerWidth.value, right, verts[2].xyz );
	VectorSubtract( start, right, verts[2].xyz );
	verts[2].st[0] = 1;
	verts[2].st[1] = 1;
...
//	VectorMA( start, cg_tracerWidth.value, right, verts[3].xyz );
	VectorAdd( start, right, verts[3].xyz );
	verts[3].st[0] = 0;
	verts[3].st[1] = 0;
...
void CG_Bullet( vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh, int fleshEntityNum ) {
	...
	if ( flesh ) {
//freeze
		if ( Q_IsFrozenBody( fleshEntityNum ) ) {
			CG_IceChip( end, fleshEntityNum );
		}
		else
//freeze
