...
int BotNumTeamMates(bot_state_t *bs) {
	...
	for ( ent = g_entities; ent < &g_entities[level.maxclients]; ent++ ) {
		...
		//
		if (BotSameTeam(bs, ent->s.number)) {
			numplayers++;
		}
	}
	return numplayers;
...
int BotSortTeamMatesByBaseTravelTime(bot_state_t *bs, int *teammates, int maxteammates) {
	...
	int j, k, numteammates, traveltime;
	...
	for ( ent = g_entities; ent < &g_entities[level.maxclients]; ent++ ) {
		...
		if (BotSameTeam(bs, ent->s.number)) {
			//
			traveltime = BotClientTravelTimeToGoal(ent->s.number, goal);
			//
...
void BotTeamOrders(bot_state_t *bs) {
	...
	for ( ent = g_entities; ent < &g_entities[level.maxclients]; ent++ ) {
		...
		//
		if (BotSameTeam(bs, ent->s.number)) {
			teammates[numteammates] = ent->s.number;
			numteammates++;
		}
	}
	//
