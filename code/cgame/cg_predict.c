...
//freeze
	// if we're going from grapple fly to grapple fly
	if ( prev->ps.pm_flags & PMF_GRAPPLE_FLY && next->ps.pm_flags & PMF_GRAPPLE_FLY ) {
		out->grappleTime = prev->ps.grappleTime + f * (next->ps.grappleTime - prev->ps.grappleTime);
	}
//freeze
}
...
static int IsUnacceptableError( playerState_t *ps, playerState_t *pps ) {
	...
	VectorSubtract( pps->grappleVelocity, ps->grappleVelocity, delta );
	if ( VectorLengthSquared( delta ) > 0.1f * 0.1f ) {
		return 7;
	}

	if ( pps->grappleTime != ps->grappleTime || pps->grappleEntity != ps->grappleEntity ) {
		return 8;
	}
	...
	if ( Q_abs(pps->viewangles[0] - ps->viewangles[0]) > 1.0f ||
			Q_abs(pps->viewangles[1] - ps->viewangles[1]) > 1.0f ||
			Q_abs(pps->viewangles[2] - ps->viewangles[2]) > 1.0f ) {
		return 14;
	}
...
//unlagged - optimized prediction
	...
	if ( cg.demoPlayback || ((cg.snap->ps.pm_flags & PMF_FOLLOW) && cg.snap->ps.pm_type != PM_FREEZE) ) {
...
//freeze
	cg_pmove.grapple = cgs.grapple;
	cg_pmove.grappleSpeed = cgs.grappleSpeed;
	cg_pmove.grapplePullSpeed = cgs.grapplePullSpeed;
	cg_pmove.reloadFactor = cgs.reloadFactor;
	cg_pmove.cgame = qtrue;
	cg_pmove.pogoJumps = cgs.pogoJumps;
//freeze
//pmove_accurate
	cg_pmove.pmove_accurate = cg_pmove_accurate.integer;
//pmove_accurate
...
//freeze
	if ( cg.pointClickScores ) {
		cg_pmove.ps->stats[STAT_FLAGS] |= SF_FREEZEANGLES;
	}
	else {
		cg_pmove.ps->stats[STAT_FLAGS] &= ~SF_FREEZEANGLES;
	}
...
//anticheat
	//cg_pmove.ps->weapon = BG_UnhashWeapon( cg_pmove.ps->stats[STAT_HASHED_WEAPON], cgs.sv_mapname, cg_pmove.cmd.serverTime );
//anticheat
	...
	for ( cmdNum = current - CMD_BACKUP + 1 ; cmdNum <= current ; cmdNum++ ) {
		...
		// set up the freeze angles for reprediction
		if ( (cg_pmove.cmd.serverTime >= cg.freezeAnglesTime && cg.freezingAngles) ||
				( cg_pmove.cmd.serverTime >= cg.freezeAnglesTime && cg_pmove.cmd.serverTime < cg.unfreezeAnglesTime) ) {
			cg_pmove.ps->stats[STAT_FLAGS] |= SF_FREEZEANGLES;
		}
		else {
			cg_pmove.ps->stats[STAT_FLAGS] &= ~SF_FREEZEANGLES;
...
//ssdemo
				int oldSpeed = cg_pmove.ps->speed;
				if ( cg_pmove.ps->persistant[PERS_TEAM] == TEAM_SPECTATOR || cg_pmove.ps->persistant[PERS_TEAM] == TEAM_REDCOACH || cg_pmove.ps->persistant[PERS_TEAM] == TEAM_BLUECOACH ) {
					cg_pmove.ps->speed /= cg_timescale.value;
				}
//ssdemo
				// run the Pmove
				Pmove (&cg_pmove);
