...
int UI_Cvar_VariableIntegerValue( const char *name ) {
	return atoi( UI_Cvar_VariableString(name) );
}
...
static void UI_SDemo( void ) {
	...
	if ( trap_Argc() != 2 ) {
		Com_Printf( "you need to supply a demo file name\n" );
		return;
	}
	...
	if ( !inDemoFile ) {
		// try it in the main/mod dir instead
		Com_sprintf( filename, sizeof(filename), "%s" SSDEMO_EXTENSION, demoname );
		trap_FS_FOpenFile( filename, &inDemoFile, FS_READ );

		if ( !inDemoFile ) {
			Com_Printf( "unable to open file %s", filename );
			return;
		}
	}

#ifdef HUMAN_READABLE_DEMOS
	// read the first directive header
	trap_FS_Read( header, 10, inDemoFile );
	header[10] = '\0';

	// a demo file MUST start with a map directive
	if ( header[0] != 'd' || header[1] != 'h' ) {
		trap_FS_FCloseFile( inDemoFile );
		Com_Printf( "invalid demo file %s\n", filename );
		return;
	}

	// get the map name size
	size = atoi( &header[2] );
#else
	// read the header
	...
	if ( dir != DIR_DEMOHEADER ) {
		trap_FS_FCloseFile( inDemoFile );
		Com_Printf( "invalid demo file %s", filename );
		return;
	}
	...
	if ( Q_stricmp(s, SSDEMO_VERSION) != 0 ) {
		// oops!
		Com_Printf( "demo version mismatch: version %s, demo version %s\n", SSDEMO_VERSION, s );
	}

	// get the map name
	s = Info_ValueForKey(info, "map");

	// set this so the demo can be picked back up
	trap_Cvar_Set( "##_continueDemoName", demoname );
	trap_Cvar_Set( "##_continueDemoLoc", "0" );

	// execute the map change (this will also start the server game)
	// use 'devmap' instead of 'map' so players will get cheats
	Com_sprintf( mapcmd, sizeof(mapcmd), "devmap %s", s );
	trap_Cmd_ExecuteText( EXEC_APPEND, mapcmd );
}
//ssdemo
