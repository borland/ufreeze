...
//multiview
		// fix up the playerstate clientNum right away
		dest->ps.clientNum = dest->ps.stats[STAT_SPEC_CLIENTNUM];
//multiview

		// FIXME: why would trap_GetSnapshot return a snapshot with the same server time
		if ( cg.snap && r && dest->serverTime == cg.snap->serverTime ) {
//anticheat
			// dunno, so I uncommented this because weapon hashing won't work otherwise
			continue;
//anticheat
		}
		...
		if ( r ) {
//anticheat
			//dest->ps.weapon = BG_UnhashWeapon( dest->ps.stats[STAT_HASHED_WEAPON], cgs.sv_mapname, dest->serverTime );
//anticheat
