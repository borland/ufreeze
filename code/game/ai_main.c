...
int BotAI_GetEntityState( int entityNum, entityState_t *state ) {
	...
	if (ent->r.svFlags & SVF_NOCLIENT && strcmp(ent->classname, "item_botroam") != 0) return qfalse;
...
void BotInputToUserCommand(bot_input_t *bi, usercmd_t *ucmd, int delta_angles[3], int time) {
	...
	if (bi->actionflags & ACTION_GRAPPLE) ucmd->buttons |= BUTTON_GRAPPLE;
...
//freeze
	// if the bot wants to grapple, set the action flag
	if (bs->grapple) {
...
int BotAIStartFrame(int time) {
	...
	trap_Cvar_Update(&bot_grapplemonkey);
...
//		trap_Cvar_Set("bot_report", "0");
	...
	if ( botlib_residual >= thinktime ) {
		...
		for (i = 0; i < MAX_GENTITIES; i++) {
			...
			if (ent->r.svFlags & SVF_NOCLIENT && strcmp(ent->classname, "item_botroam") != 0) {
				trap_BotLibUpdateEntity(i, NULL);
				continue;
			}
