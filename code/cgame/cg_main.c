...
//freeze
int swapSkinsModificationCount = -1;
//freeze
//pmskins
int enemyColorsModificationCount = -1;
int colorsModificationCount = -1;
int forceEnemyPMSkinsModificationCount = -1;
int forceTeamPMSkinsModificationCount = -1;
//pmskins
...
#endif
//freeze
vmCvar_t	cg_iceChips;
vmCvar_t	cg_weather;
vmCvar_t	cg_noVoiceChatSounds;
vmCvar_t	cg_chatFilter;
vmCvar_t	cg_drawFrozen;
...
vmCvar_t	cg_lastCue;
vmCvar_t	cg_refLogin;
vmCvar_t	cg_noChatBeep;
vmCvar_t	cg_noTeamChatBeep;
vmCvar_t	cg_noGrappleSound;
vmCvar_t	cg_noStats;
vmCvar_t	cg_otherRailTrailTime;
vmCvar_t	cg_drawSpeed;
vmCvar_t	cg_crosshairPulse;
vmCvar_t	cg_crosshairColor;
vmCvar_t	cg_centerPrintTime;
vmCvar_t	cg_pointClickSpec;
vmCvar_t	cg_sensitivity;
...
//freeze
//pmove_accurate
vmCvar_t	cg_pmove_accurate;
//pmove_accurate
//pmskins
vmCvar_t	cg_enemyColors;
vmCvar_t	cg_colors;
vmCvar_t 	cg_forceEnemyPMSkins;
vmCvar_t 	cg_forceTeamPMSkins;
//pmskins
//nomip
vmCvar_t	cg_nomip;
//nomip
...
//anticheat
vmCvar_t	cm_noAreas;
vmCvar_t	cm_noCurves;
vmCvar_t	cm_playerCurveClip;
vmCvar_t	debuggraph;
vmCvar_t	fixedtime;
vmCvar_t	graphheight;
vmCvar_t	graphscale;
vmCvar_t	graphshift;
vmCvar_t	r_clear;
...
vmCvar_t	r_drawentities;
vmCvar_t	r_drawworld;
vmCvar_t	r_flareFade;
vmCvar_t	r_flaresSize;
vmCvar_t	r_fullbright;
vmCvar_t	r_lockpvs;
vmCvar_t	r_lodscale;
vmCvar_t	r_logFile;
vmCvar_t	r_measureOverdraw;
vmCvar_t	r_nobind;
vmCvar_t	r_nocull;
vmCvar_t	r_nocurves;
vmCvar_t	r_nolightcalc;
vmCvar_t	r_noportals;
vmCvar_t	r_norefresh;
vmCvar_t	r_novis;
vmCvar_t	r_offsetfactor;
vmCvar_t	r_offsetunits;
vmCvar_t	r_showcluster;
vmCvar_t	r_showImages;
vmCvar_t	r_shownormals;
vmCvar_t	r_showsky;
vmCvar_t	r_showSmp;
vmCvar_t	r_showtris;
vmCvar_t	r_singleShader;
vmCvar_t	r_skipBackEnd;
vmCvar_t	r_speeds;
vmCvar_t	r_verbose;
vmCvar_t	r_znear;
vmCvar_t	s_show;
vmCvar_t	s_testsound;
vmCvar_t	timedemo;
vmCvar_t	timegraph;
vmCvar_t	timescale;
vmCvar_t	viewlog;
...
freeze*/
	...
	{ &cg_thirdPersonHeight, "cg_thirdPersonHeight", "70", CVAR_CHEAT },
	{ &cg_thirdPersonAngle, "cg_thirdPersonAngle", "0", CVAR_CHEAT },
//freeze
...
//freeze
	{ &cg_iceChips, "cg_iceChips", "1", CVAR_ARCHIVE},
	{ &cg_weather, "cg_weather", "1", CVAR_ARCHIVE},
	{ &cg_noVoiceChatSounds, "cg_noVoiceChatSounds", "0", CVAR_ARCHIVE },
	{ &cg_chatFilter, "cg_chatFilter", "0", CVAR_ARCHIVE },
	{ &cg_drawFrozen, "ch_drawFrozen", "1", CVAR_ARCHIVE },
	{ &cg_enemyModel, "cg_enemyModel", "sarge/default", CVAR_ARCHIVE },
	{ &cg_enemyHeadModel, "cg_enemyHeadModel", "sarge/default", CVAR_ARCHIVE },
	{ &cg_swapSkins, "cg_swapSkins", "0", 0 },
	{ &cg_teamRails, "cg_teamRails", "0", CVAR_ARCHIVE },
	{ &cg_lastCue, "cg_lastCue", "1", CVAR_ARCHIVE },
	{ &cg_refLogin, "ref_login", "none", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &cg_noChatBeep, "cg_noChatBeep", "0", CVAR_ARCHIVE },
	{ &cg_noTeamChatBeep, "cg_noTeamChatBeep", "0", CVAR_ARCHIVE },
	{ &cg_noGrappleSound, "cg_noGrappleSound", "0", CVAR_ARCHIVE },
	{ &cg_noStats, "cg_noStats", "0", CVAR_ARCHIVE | CVAR_USERINFO },
	{ &cg_otherRailTrailTime, "cg_otherRailTrailTime", "400", CVAR_ARCHIVE },
	{ &cg_drawSpeed, "ch_drawSpeed", "0", CVAR_ARCHIVE },
	{ &cg_crosshairPulse, "cg_crosshairPulse", "1", CVAR_ARCHIVE },
	{ &cg_crosshairColor, "ch_crosshairColor", "white", CVAR_ARCHIVE },
	{ &cg_centerPrintTime, "cg_centerPrintTime", "3.0", CVAR_ARCHIVE },
	{ &cg_pointClickSpec, "cg_pointClickSpec", "1", CVAR_ARCHIVE },
	{ &cg_sensitivity, "sensitivity", "5", CVAR_ARCHIVE },
	{ &cg_autodemo, "cg_autodemo", "1", CVAR_ARCHIVE },
	{ &cg_autoscreenshot, "cg_autoscreenshot", "1", CVAR_ARCHIVE },
	{ &m_pitch, "m_pitch", "0.022", CVAR_ARCHIVE },
	{ &m_yaw, "m_yaw", "0.022", CVAR_ARCHIVE },
	{ &cg_showPlayerLean, "cg_showPlayerLean", "1", CVAR_ARCHIVE },
//freeze
//pmove_accurate
	{ &cg_pmove_accurate, "cg_pmove_accurate", "1", CVAR_USERINFO | CVAR_ARCHIVE },
//pmove_accurate
//pmskins
	{ &cg_enemyColors, "cg_enemyColors", "green ltgreen ltgreen ltgreen", CVAR_ARCHIVE },
	{ &cg_colors, "cg_colors", "white ltblue ltblue blue", CVAR_ARCHIVE },
	{ &cg_forceEnemyPMSkins, "cg_forceEnemyPMSkins", "0", CVAR_ARCHIVE },
	{ &cg_forceTeamPMSkins, "cg_forceTeamPMSkins", "0", CVAR_ARCHIVE },
//pmskins
//nomip
	{ &cg_nomip, "cg_nomip", "0", CVAR_ARCHIVE | CVAR_LATCH }, // must vid_restart to see changes
//nomip
...
//unlagged - client options
	...
	{ &cg_truePing, "cg_truePing", "0", CVAR_ARCHIVE },
...
//unlagged - client options
//anticheat
	{ &cm_noAreas, "cm_noAreas", "0", CVAR_CHEAT },
	{ &cm_noCurves, "cm_noCurves", "0", CVAR_CHEAT },
	{ &cm_playerCurveClip, "cm_playerCurveClip", "1", CVAR_CHEAT },
	{ &debuggraph, "debuggraph", "0", CVAR_CHEAT },
	{ &fixedtime, "fixedtime", "0", CVAR_CHEAT },
	{ &graphheight, "graphheight", "32", CVAR_CHEAT },
	{ &graphscale, "graphscale", "1", CVAR_CHEAT },
	{ &graphshift, "graphshift", "0" , CVAR_CHEAT },
	...
	{ &r_flaresSize, "r_flaresSize", "40", CVAR_CHEAT },
	{ &r_fullbright, "r_fullbright", "0", CVAR_CHEAT },
	...
	{ &r_nolightcalc, "r_nolightcalc", "0", CVAR_CHEAT },
	...
	{ &r_showImages, "r_showImages", "0", CVAR_CHEAT },
	...
	{ &r_singleShader, "r_singleShader", "0", CVAR_CHEAT },
	...
	{ &s_show, "s_show", "0", CVAR_CHEAT },
	{ &s_testsound, "s_testsound", "0", CVAR_CHEAT },
	{ &viewlog, "viewlog", "0", CVAR_CHEAT },
//anticheat
};

const int randomCrashEnd = 0;

static int  cvarTableSize = sizeof( cvarTable ) / sizeof( cvarTable[0] );

qboolean CG_Cvar_Clamp( vmCvar_t *vmCvar, float min, float max ) {
	if ( vmCvar->value > max ) {
		CG_Printf( "Allowable values are %.2f to %.2f.\n", min, max );

		Com_sprintf( vmCvar->string, MAX_CVAR_VALUE_STRING, "%f", max );
		vmCvar->value = max;
		vmCvar->integer = (int)max;

		trap_Cvar_Set( cvarTable[vmCvar->index].cvarName, vmCvar->string );
		return qtrue;
	}
...
qboolean CG_Cvar_ClampInt( vmCvar_t *vmCvar, int min, int max ) {
	if ( vmCvar->integer > max ) {
		CG_Printf( "Allowable values are %d to %d.\n", min, max );
...
//anticheat
void CG_Cvar_Update( vmCvar_t *vmCvar, int grace ) {
	int flags;
	qboolean demo = ( cg.demoPlayback || cg.ssDemoPlayback );
	
	flags = cvarTable[vmCvar->index].cvarFlags;
	if ( !cgs.sv_cheats && !demo && ( flags & CVAR_CHEAT ) ) {
		char *def = cvarTable[vmCvar->index].defaultString;
		if ( strcmp( vmCvar->string, def ) != 0 ) {
			char *name = cvarTable[vmCvar->index].cvarName;
			if ( grace || cg.randomcrash != 0 || (cg.predictedPlayerState.stats[STAT_FLAGS] & SF_PRIVATE) ||
				(cg.predictedPlayerState.stats[STAT_FLAGS] & SF_LOCAL) ) {
				CG_Printf( "%s is cheat protected.\n" );
			}
			else {
				...
				CG_Printf( "%s is cheat protected.  Your attempt has been reported to the server administrator.\n", name );
				trap_SendConsoleCommand( va("%s %s \"%s\"\n", charkey, name, vmCvar->string) );
			}

			Q_strncpyz( vmCvar->string, def, MAX_CVAR_VALUE_STRING );
			trap_Cvar_Set( name, def );
			return;
		}
	}

	if ( !cgs.sv_cheats && !demo ) {
		if ( vmCvar == &cg_thirdPersonRange ) {
			CG_Cvar_Clamp( vmCvar, 80.0f, 120.0f );
		}
		else if ( vmCvar == &cg_thirdPerson ) {
			if ( !cgs.allowThirdPerson && atoi( vmCvar->string ) ) {
				CG_Printf( "Third-person view has been disabled on this server.\n" );

				strcpy( vmCvar->string, "0" );
				vmCvar->value = 0.0f;
				vmCvar->integer = 0;
	...
	if ( vmCvar == &cg_otherRailTrailTime ) {
		CG_Cvar_ClampInt( vmCvar, 0, 400 );
	}
	else if ( vmCvar == &cg_weather ) {
		CG_Cvar_ClampInt( vmCvar, 0, 16 );
	}
	else if ( vmCvar == &cg_cmdTimeNudge ) {
		CG_Cvar_ClampInt( vmCvar, 0, 999 );
	}
	else if ( vmCvar == &cl_timeNudge ) {
		CG_Cvar_ClampInt( vmCvar, -50, 50 );
	}
	else if ( vmCvar == &cg_latentSnaps ) {
		CG_Cvar_ClampInt( vmCvar, 0, 10 );
	}
	else if ( vmCvar == &cg_latentCmds ) {
		CG_Cvar_ClampInt( vmCvar, 0, MAX_LATENT_CMDS - 1 );
	}
	else if ( vmCvar == &cg_plOut ) {
		CG_Cvar_ClampInt( vmCvar, 0, 100 );
	}
...
//freeze
	swapSkinsModificationCount = cg_swapSkins.modificationCount;
//freeze
//pmskins
	enemyColorsModificationCount = cg_enemyColors.modificationCount;
	colorsModificationCount = cg_colors.modificationCount;
	forceEnemyPMSkinsModificationCount = cg_forceEnemyPMSkins.modificationCount;
	forceTeamPMSkinsModificationCount = cg_forceTeamPMSkins.modificationCount;
//pmskins
...
Reloads all client info for every connected, valid client
*/
static void CG_ForceInfoLoad( void ) {
	clientInfo_t *ci;
	int i;
...
Parses a four-color string into the given color vectors.  Strings are in the form:

<railcolor> <headcolor> <bodycolor> <legscolor>

where the colors are single-digit numbers, a hex string in 0xRRGGBB format, or one
of these:
...
ltgray/ltgrey
*/
static void CG_ParseColorsVar( char *colors, vec3_t rail, vec3_t head, vec3_t body, vec3_t legs ) {
	char work[MAX_STRING_CHARS];
	char *curr, *word;
	int nulls, nullnum, wordnum;
	vec_t *map[4];
	map[0] = rail;
	map[1] = head;
	map[2] = body;
	...
	if(strlen(colors) == 4){
		...
		for(wordnum = 0;wordnum<4;wordnum++){
			Com_sprintf(work,MAX_STRING_CHARS,"%c",colors[wordnum]);
			String2RGB( work, map[wordnum] );
		}
		return;
	}

	Q_strncpyz( work, colors, sizeof(work) );
	...
	curr = word = work;
	nullnum = wordnum = 0;
	for ( ; nullnum < nulls && wordnum < 4; curr++ ) {
		if ( *curr == '\0' ) {
			if ( *(curr + 1) != '\0' || nullnum == nulls - 1 ) {
				String2RGB( word, map[wordnum] );
				word = curr + 1;
				wordnum++;
			}
			nullnum++;
	...
	for ( ; wordnum < 4; wordnum++ ) {
		VectorClear( map[wordnum] );
	}
}
//pmskins
...
//anticheat
	...
	for ( i = 0, cv = cvarTable ; i < cvarTableSize ; i++, cv++ ) {
//anticheat
		CG_Cvar_Update( cv->vmCvar, grace );
//anticheat
	}
...
//freeze
	if ( swapSkinsModificationCount != cg_swapSkins.modificationCount ) {
		swapSkinsModificationCount = cg_swapSkins.modificationCount;
		CG_ForceInfoLoad();
	}
//freeze
//pmskins
	if ( enemyColorsModificationCount != cg_enemyColors.modificationCount ) {
		...
		CG_ParseColorsVar( cg_enemyColors.string, cgs.enemyRailColor,
				cgs.enemyHeadColor, cgs.enemyBodyColor, cgs.enemyLegsColor );
	}
	
	if ( colorsModificationCount != cg_colors.modificationCount ) {
		colorsModificationCount = cg_colors.modificationCount;

		CG_ParseColorsVar( cg_colors.string, cgs.railColor,
				cgs.headColor, cgs.bodyColor, cgs.legsColor );
	}

	if ( forceEnemyPMSkinsModificationCount != cg_forceEnemyPMSkins.modificationCount ) {
		forceEnemyPMSkinsModificationCount = cg_forceEnemyPMSkins.modificationCount;
		CG_ForceInfoLoad();
	}

	if ( forceTeamPMSkinsModificationCount != cg_forceTeamPMSkins.modificationCount ) {
		forceTeamPMSkinsModificationCount = cg_forceTeamPMSkins.modificationCount;
		CG_ForceInfoLoad();
	}
//pmskins
...
//freeze - client-side chat profanity filter
...
ID_INLINE qboolean filter_charmatches( char c1, char c2 ) {
	c1 = toupper( c1 );
	c2 = toupper( c2 );
...
ID_INLINE qboolean filter_isletter( char c ) {
	if ( c >= 'A'  && c <= 'Z' )
		return qtrue;
...
qboolean CG_FilterString( char *str, char **words, int numWords ) {
...
qboolean CG_FilterChat( char *str ) {
	int ret = CG_FilterString( str, filteredWords, nFilteredWords );
	// These are used to simulate VM conditions (slow)
	// Comment them out for production!
/*
	CG_FilterString( str, filteredWords, nFilteredWords, cgs.filteredWords, cgs.numFilteredWords );
	CG_FilterString( str, filteredWords, nFilteredWords, cgs.filteredWords, cgs.numFilteredWords );
	CG_FilterString( str, filteredWords, nFilteredWords, cgs.filteredWords, cgs.numFilteredWords );
	CG_FilterString( str, filteredWords, nFilteredWords, cgs.filteredWords, cgs.numFilteredWords );
	CG_FilterString( str, filteredWords, nFilteredWords, cgs.filteredWords, cgs.numFilteredWords );
...
void QDECL CG_Printf( const char *msg, ... ) {
	...
	//NT - chat filter
	if ( cg_chatFilter.integer ) {
		// try to determine if it's chat
		int i;
		for ( i = 0; text[i] != '\0'; i++) {
			if ( text[i] == ':' && text[i + 1] == ' ' )
				break;
		...
		// if we found a colon-space
		if ( text[i] != '\0' ) {
			CG_FilterChat( &text[i] );
		}
	}
...
//freeze
		cgs.media.lastPlayerSound = trap_S_RegisterSound( "sound/teamplay/flagtk_blu.wav", qtrue );
		cgs.media.timeoutSound = trap_S_RegisterSound( "sound/teamplay/whistle.wav", qtrue );
		cgs.media.freezingSound = trap_S_RegisterSound( "sound/teamplay/freezing.wav", qtrue );
		cgs.media.grappleFireSound = trap_S_RegisterSound( "sound/weapons/grapple/grapplefire.wav", qtrue );
		cgs.media.pogoSound = trap_S_RegisterSound( "sound/misc/pogo.wav", qtrue );
//nomip
		if ( cg_nomip.integer & NOMIP_LIGHTNING )
			cgs.media.lightningShader = trap_R_RegisterShader( "lightningBoltNew_nomip" );
		else
//nomip
			cgs.media.lightningShader = trap_R_RegisterShader( "lightningBoltNew" );
		cgs.media.grappleCylinder = trap_R_RegisterModel( "models/weapons2/grapple/cylinder.md3" );

		cgs.media.menu_in_sound	= trap_S_RegisterSound( "sound/misc/menu1.wav", qfalse );
		cgs.media.menu_move_sound	= trap_S_RegisterSound( "sound/misc/menu2.wav", qfalse );
		cgs.media.menu_out_sound	= trap_S_RegisterSound( "sound/misc/menu3.wav", qfalse );
		cgs.media.menu_buzz_sound	= trap_S_RegisterSound( "sound/misc/menu4.wav", qfalse );
//freeze
...
static void CG_RegisterGraphics( void ) {
	...
	cgs.media.scoreboardTime = trap_R_RegisterShaderNoMip( "menu/tab/time2.tga" );
//freeze
	cgs.media.scoreboardFrozen = trap_R_RegisterShaderNoMip( "menu/tab/frozen.tga" );
//freeze

//nomip
	if ( cg_nomip.integer & NOMIP_SMOKE ) {
...
#endif
//nomip
	if ( cg_nomip.integer & NOMIP_PLASMA )
...
//freeze
		...
		cgs.media.crosshairShader2[i] = trap_R_RegisterShader( va("gfx/3d/crosshair%c", 'a'+i) );
//freeze
	}

	cgs.media.spectatorCrosshairShader = trap_R_RegisterShader( "spectatorcrosshair" );
	cgs.media.spectatorPointerShader = trap_R_RegisterShader( "spectatorpointer" );
	cgs.media.spectatorPointerXShader = trap_R_RegisterShader( "spectatorpointerx" );
	cgs.media.spectatorPointerMaxShader = trap_R_RegisterShader( "spectatorpointermax" );
	cgs.media.spectatorMoveShader = trap_R_RegisterShader( "spectatormove" );
	cgs.media.spectatorResizeNSShader = trap_R_RegisterShader( "spectatorresizens" );
	cgs.media.spectatorResizeEWShader = trap_R_RegisterShader( "spectatorresizeew" );
	cgs.media.spectatorResizeNWSEShader = trap_R_RegisterShader( "spectatorresizenwse" );
	cgs.media.spectatorResizeNESWShader = trap_R_RegisterShader( "spectatorresizenesw" );
	cgs.media.pipWindowXShader = trap_R_RegisterShader( "pipwindowx" );
	cgs.media.pipWindowMaxShader = trap_R_RegisterShader( "pipwindowmax" );
...
//freeze
	...
	cgs.media.invisShader = trap_R_RegisterShader("invisibility");
	cgs.media.invisFiringShader = trap_R_RegisterShader("invisibility_firing");
//freeze
	cgs.media.regenShader = trap_R_RegisterShader("powerups/regen" );
//nomip
	if ( cg_nomip.integer & NOMIP_SMOKE )
		cgs.media.hastePuffShader = trap_R_RegisterShader("hasteSmokePuff_nomip" );
	else
//nomip
		cgs.media.hastePuffShader = trap_R_RegisterShader("hasteSmokePuff" );
//freeze
	cgs.media.freezeShader = trap_R_RegisterShader( "freezeShader" );
	cgs.media.freezeShader_a = trap_R_RegisterShader( "freezeShader_a" );
	cgs.media.freezeShader_b = trap_R_RegisterShader( "freezeShader_b" );
	cgs.media.freezeShader2 = trap_R_RegisterShader( "freezeShader2" );
	cgs.media.freezeMarkShader = trap_R_RegisterShader( "freezeMarkShader" );
//nomip
	if ( cg_nomip.integer & NOMIP_RAILGUN )
		cgs.media.railCoreShader = trap_R_RegisterShader( "railCore_nomip" );
	else
//nomip
		cgs.media.railCoreShader = trap_R_RegisterShader( "railCore" );
	cgs.media.snowflakeShader = trap_R_RegisterShader( "snowflake" );
	cgs.media.rainShader = trap_R_RegisterShader( "rain" );
	cgs.media.bboxShader = trap_R_RegisterShader( "bbox" );
	cgs.media.bboxShader_nocull = trap_R_RegisterShader( "bbox_nocull" );
	cgs.media.quadShader_nocull = trap_R_RegisterShader( "quad_nocull" );
	cgs.media.grappleFlareShader = trap_R_RegisterShader( "grappleFlare" );
//freeze
...
//nomip
	if ( cg_nomip.integer & NOMIP_BLOOD )
...
//nomip
	if ( cg_nomip.integer & NOMIP_BULLETSEXPL )
		cgs.media.bulletFlashModel = trap_R_RegisterModel("models/weaphits/bullet_nomip.md3");
	else
//nomip
	...
	for ( i = 1 ; i < bg_numItems ; i++ ) {
		if ( items[ i ] == '1' || cg_buildScript.integer ) {
			if ( bg_itemlist[i].giType != IT_INVISIBLE ) {
				CG_LoadingItem( i );
...
//freeze
	CG_RegisterItemVisuals( BG_FindItemForPowerup( PW_INVIS ) - bg_itemlist );
	CG_RegisterItemVisuals( BG_FindItemForPowerup( PW_SIGHT ) - bg_itemlist );
//freeze
...
void CG_Init( int serverMessageNum, int serverCommandSequence, int clientNum ) {
	...
	memset( cg_items, 0, sizeof(cg_items) );
//multiview
	CG_InitializeRefEntities();
	CG_InitializeWindowRequests();
//multiview

	cg.clientNum = clientNum;
//freeze
	// make this available to the UI game
	trap_Cvar_Set( "cg_clientNum", va("%d", cg.clientNum) );

	// write out Description.txt to keep it up-to-date
...
//freeze
	// discovered this bug in PIP management - should have fixed it before though
