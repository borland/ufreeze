...
vmCvar_t bot_grapplemonkey;
vmCvar_t bot_chatfrag;
...
int BotTeam(bot_state_t *bs) {
	...
	//
	if ( g_entities[bs->client].client->sess.sessionTeam == TEAM_RED ) return TEAM_RED;
	else if ( g_entities[bs->client].client->sess.sessionTeam == TEAM_BLUE ) return TEAM_BLUE;
...
==================
*/
//freeze - used to return qboolean
float EntityIsInvisible(bot_state_t *bs, aas_entityinfo_t *entinfo) {
	// the flag is always visible
	if (EntityCarriesFlag(entinfo)) {
		return 0.0f;
	}
//freeze
	if ( (entinfo->powerups & (1 << PW_SIGHT)) || bs->inventory[INVENTORY_SIGHT] ) {
		return 0.0f;
	}
//freeze
	if (entinfo->powerups & (1 << PW_INVIS)) {
//freeze
		gentity_t *ent = &g_entities[entinfo->number];
		float speed = VectorLength(ent->s.pos.trDelta);
		float speedinvis = 1.0f, firinginvis = 1.0f;

		if ( speed > 160.0f ) {
			speedinvis = 1.0f - Com_Clamp(0.0f, 1.0f, (speed - (g_speed.value / 2.0f)) / g_speed.value);
		}

		// cheat a bit on the firing
		if ( level.time - ent->lastFiringTime < 2000 ) {
			firinginvis = Com_Clamp(0.0f, 1.0f, (float)(level.time - ent->lastFiringTime) / 2000.0f);
		}

		//G_Printf("speedinvis: %.2f, firinginvis: %.2f\n", speedinvis, firinginvis);

//freeze
		return min(speedinvis, firinginvis);
	}
	return 0.0f;
}
...
//freeze
		...
		if (client >= 0 && client < MAX_GENTITIES) {
			gentity_t *ent = &g_entities[client];
			if (is_body_freeze(ent) && ent->target_ent) {
				client = ent->target_ent->s.number;
				found = qtrue;
			}
		}
...
void BotSetupForMovement(bot_state_t *bs) {
	...
	//
//freeze
	// if the bot wants to sneak, walk
	if (bs->sneaky) {
...
//freeze
	bs->inventory[INVENTORY_SIGHT] = bs->cur_ps.powerups[PW_SIGHT] != 0;
//freeze
...
qboolean BotIsObserver(bot_state_t *bs) {
	if ( bs->client < 0 || bs->client >= level.maxclients ) return qtrue;

	if (bs->cur_ps.pm_type == PM_SPECTATOR) return qtrue;

	if ( g_entities[bs->client].client->sess.sessionTeam == TEAM_SPECTATOR || 
		 g_entities[bs->client].client->sess.sessionTeam == TEAM_BLUECOACH ||
		  g_entities[bs->client].client->sess.sessionTeam == TEAM_REDCOACH) return qtrue;
	return qfalse;
}
...
//freeze
	// if going to thaw a teammate (or thawing)
	if ( bs->ltgtype == LTG_TEAMACCOMPANY && !bs->ordered ) {
		return qtrue;
	}

	// if trying to be sneaky
	if ( bs->sneaky ) {
		//return qtrue;
	}
//freeze
...
//freeze
	...
	// don't camp if on the way to thaw a teammate
	if (bs->ltgtype == LTG_TEAMACCOMPANY && !bs->ordered) {
		return qfalse;
	}

	// don't camp if half the team is out of commission
	if (BotHalfTeamIsOOC(bs)) {
		return qfalse;
	}

	// a seer never wants to camp (bad karma)
	if (bs->inventory[INVENTORY_SIGHT]) {
		return qfalse;
	}

	// we'll always be campy if we want to be sneaky and we have nothing to do
	campy = bs->sneaky && BotNumTeamFrozen(bs) == 0;
//freeze
...
//freeze
	...
	if (bs->camp_time > FloatTime() - 60 + 300 * (1-camper) && !campy) return qfalse;
	//
	//if (random() > camper) {
	if (random() > camper && !campy) {
//freeze
		bs->camp_time = FloatTime();
		return qfalse;
	}
...
int BotSameTeam(bot_state_t *bs, int entnum) {
	...
	if ( gametype >= GT_TEAM ) {
		if ( g_entities[bs->client].client->sess.sessionTeam == g_entities[entnum].client->sess.sessionTeam ) return qtrue;
	}
	return qfalse;
}
...
//freeze
	bs->healthdelta = bs->inventory[INVENTORY_HEALTH] - bs->lasthealth;
	if ( bs->healthdelta < -1 ) {
		bs->healthdecreasetime = FloatTime();
	}
//freeze
...
#endif
//freeze
	if ( curenemy >= 0 && bs->stopfighting ) {
		bs->stopfighting = qfalse;
		bs->enemy = -1;
		return qfalse;
	}
//freeze

	//G_Printf("client %d, healthdelta: %d; ", bs->client, bs->healthdelta);
	//
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		//G_Printf("considering %d: ", i);

		//if it's the current enemy
		if (i == curenemy) {
			//G_Printf("current enemy\n");
			continue;
		}
		...
		if (EntityIsInvisible(bs, &entinfo) > 0.9f) {
			//G_Printf("invisible\n");
			continue;
		}
//unlagged - misc
...
//unlagged - misc
		...
		if ((easyfragger < 0.5 || !bot_chatfrag.integer) && EntityIsChatting(&entinfo)) {
			//G_Printf("chatting\n");
			continue;
		}
		//
...
//freeze
		// sneaky bots that haven't taken damage in more than 2 seconds have more criteria
		if (bs->sneaky && FloatTime() - bs->healthdecreasetime > 2.0f) {
			vec3_t delta, dir, deltadir;

			//G_Printf("extra criteria: ");
			...
			// if the enemy is facing the direction of probable attack
			if (DotProduct(deltadir, dir) < 0.0f) {
				//G_Printf("can't shoot in back\n");
				// don't attack
				continue;				
			}
			// otherwise, go ahead and shoot him in the back
			bs->sneaky = qfalse;
...
void BotAimAtEnemy(bot_state_t *bs) {
	...
	if (EntityIsInvisible(bs, &entinfo) > 0.0f) {
		aim_accuracy *= 1.0f - EntityIsInvisible(bs, &entinfo);
	}
	//
...
void BotCheckForGrenades(bot_state_t *bs, entityState_t *state) {
	gentity_t *ent = &g_entities[state->number];

	// if this is a grenade
	if (ent->s.eType == ET_MISSILE && ent->s.weapon == WP_GRENADE_LAUNCHER) {
		// try to avoid the grenade
		trap_BotAddAvoidSpot(bs->ms, ent->s.pos.trBase, 160, AVOID_ALWAYS);
		return;
	}

	// if an enemy corpsicle
	if (is_body_freeze(ent) && ent->target_ent && !BotSameTeam(bs, ent->target_ent->s.number)) {
		// try to avoid it to keep from getting stuck
		trap_BotAddAvoidSpot(bs->ms, ent->s.pos.trBase, 60, AVOID_ALWAYS);
		return;
	}
}
...
void BotSetupDeathmatchAI(void) {
	...
	trap_Cvar_Register(&bot_rocketjump, "bot_rocketjump", "1.0", 0);
	//trap_Cvar_Register(&bot_grapple, "bot_grapple", "1.0", CVAR_ARCHIVE);
	trap_Cvar_Register(&bot_grapplemonkey, "bot_grapplemonkey", "2.0", 0);
	trap_Cvar_Register(&bot_chatfrag, "bot_chatfrag", "1", 0);
