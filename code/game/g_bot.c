...
void G_AddRandomBot( int team ) {
	...
	for ( n = 0; n < g_numBots ; n++ ) {
		...
		for ( i = 0; i < g_maxclients.integer; i++ ) {
			...
			if ( !Q_stricmp( value, ent->client->pers.netname ) ) {
				break;
			}
		}
...
//pmskins
	...
	if( !team || !*team ) {
		if( g_gametype.integer >= GT_TEAM ) {
			int teamnum = PickTeam(clientNum);
			if ( teamnum == TEAM_RED) {
				team = "red";
			}
...
//freeze
	if ( !Q_stricmp(team, "red") && level.redlocked ) {
		if ( level.bluelocked ) {
			team = "spectator";
		}
		else {
	...
	else if ( !Q_stricmp(team, "blue") && level.bluelocked ) {
		if ( level.redlocked ) {
			team = "spectator";
		}
		else {
