...
//freeze
	...
	for ( iter = g_entities; iter < &g_entities[level.maxclients]; iter++ ) {
		if ( iter->inuse && iter->client && iter->client->sess.sessionTeam == TEAM_SPECTATOR &&
			...
			iter->client->bestAutofollow = other;
			iter->client->autofollowTime = level.time + 1000; // give it some extra time
		}
	}
...
void Touch_Item (gentity_t *ent, gentity_t *other, trace_t *trace) {
	...
	other->client->lastItem = ent->item->pickup_name;
...
//freeze
	...
	if ( ent->item->giType == IT_INVISIBLE ) {
		ent->r.svFlags |= SVF_NOCLIENT;
	}
	else {
		ent->r.contents = CONTENTS_TRIGGER;
...
void SaveRegisteredItems( void ) {
	...
	G_SetConfigstring(CS_ITEMS, string); //ssdemo
}
...
//freeze
	if ( level.timein > level.time ) {
		...
		if ( ent->think == Body_think ) {
			ent->timestamp += level.time - level.previousTime;
			if ( ent->unfreezer ) {
				ent->count += level.time - level.previousTime;
			}
		}
