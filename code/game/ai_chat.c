...
int BotNumActivePlayers(void) {
	...
	for ( ent = g_entities; ent < &g_entities[level.maxclients]; ent++ ) {
		...
		if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR || 
			...
			ent->client->sess.sessionTeam == TEAM_BLUECOACH) continue;
		//
		num++;
	}
	return num;
...
int BotIsFirstInRankings(bot_state_t *bs) {
	...
	for ( ent = g_entities; ent < &g_entities[level.maxclients]; ent++ ) {
		...
		if ( ent->client->sess.sessionTeam == TEAM_SPECTATOR || 
			...
			ent->client->sess.sessionTeam == TEAM_BLUECOACH ) continue;
		//
		if (score < ent->client->ps.persistant[PERS_SCORE]) return qfalse;
	}
	return qtrue;
}
...
char *BotFirstClientInRankings(void) {
	...
	for ( ent = g_entities; ent < &g_entities[level.maxclients]; ent++ ) {
		...
		if (ent->client->sess.sessionTeam == TEAM_SPECTATOR || 
			...
			ent->client->sess.sessionTeam == TEAM_BLUECOACH) continue;
		//
		if ( ent->client->ps.persistant[PERS_SCORE] > bestscore) {
			bestscore = ent->client->ps.persistant[PERS_SCORE];
			bestclient = ent->s.number;
		}
	}
...
char *BotLastClientInRankings(void) {
	...
	for ( ent = g_entities; ent < &g_entities[level.maxclients]; ent++ ) {
		...
		if (ent->client->sess.sessionTeam == TEAM_SPECTATOR || 
			...
			ent->client->sess.sessionTeam == TEAM_BLUECOACH) continue;
		//
		if ( ent->client->ps.persistant[PERS_SCORE] < worstscore ) {
			worstscore = ent->client->ps.persistant[PERS_SCORE];
			bestclient = ent->s.number;
		}
	}
	EasyClientName(bestclient, name, 32);
...
char *BotRandomOpponentName(bot_state_t *bs) {
	...
	for (i = 0; i < maxclients && i < MAX_CLIENTS; i++) {
		...
		if (g_entities[i].client->sess.sessionTeam == TEAM_SPECTATOR ||
			g_entities[i].client->sess.sessionTeam == TEAM_REDCOACH ||
			g_entities[i].client->sess.sessionTeam == TEAM_BLUECOACH) continue;
		//skip team mates
...
int BotVisibleEnemies(bot_state_t *bs) {
	...
	for (i = 0; i < MAX_CLIENTS; i++) {
		...
		//if the enemy is mostly invisible
		if (EntityIsInvisible(bs, &entinfo) > 0.7f) {
			continue;
		}
		//if on the same team
		if (BotSameTeam(bs, i)) continue;
	...
	else
	{
		...
		if (bs->botdeathtype == MOD_WATER)
			...
			BotAI_BotInitialChat(bs, "death_cratered", BotRandomOpponentName(bs), NULL);
				...
				bs->botdeathtype == MOD_SUICIDE || bs->botdeathtype == MOD_KILL || 
