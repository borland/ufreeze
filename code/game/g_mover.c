...
void Reached_BinaryMover( gentity_t *ent ) {
	...
	if ( ent->moverState == MOVER_1TO2 ) {
		...
		if ( ent->teammaster == ent || !ent->teammaster ) {
			G_AdjustAreaPortalState( ent, qfalse ); //ssdemo
		}
	} else {
...
void Use_BinaryMover( gentity_t *ent, gentity_t *other, gentity_t *activator ) {
	...
	if ( ent->moverState == MOVER_POS1 ) {
		...
		// open areaportal
		if ( ent->teammaster == ent || !ent->teammaster ) {
			G_AdjustAreaPortalState( ent, qtrue ); //ssdemo
		}
		return;
	}
...
//freeze
	if ( other->lastdoortriggertouch > level.time - 1000 ) {
		return;
	}
//freeze
...
//freeze
	other->lastdoortriggertouch = level.time;
//freeze
}
...
freeze*/
//ssdemo
	//if ( other->client && is_spectator( other->client ) ) {
	if ( other->client && (level.inDemoFile != 0 || is_spectator(other->client)) ) {
//ssdemo
		if ( other->client->sess.spectatorState == SPECTATOR_FROZEN ) {
			return;
		}
//freeze
