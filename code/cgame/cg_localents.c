...
void CG_AddFragment( localEntity_t *le ) {
	...
	if ( le->pos.trType == TR_STATIONARY ) {
		...
		if ( t < SINK_TIME ) {
//freeze
			if ( le->leType == LE_ICEFRAGMENT ) {
				vec3_t oldaxis[3];

				VectorCopy( le->refEntity.axis[0], oldaxis[0] );
				VectorCopy( le->refEntity.axis[1], oldaxis[1] );
				VectorCopy( le->refEntity.axis[2], oldaxis[2] );

				VectorScale( le->refEntity.axis[0], (float)t / SINK_TIME, le->refEntity.axis[0] );
				VectorScale( le->refEntity.axis[1], (float)t / SINK_TIME, le->refEntity.axis[1] );
				VectorScale( le->refEntity.axis[2], (float)t / SINK_TIME, le->refEntity.axis[2] );

				CG_AddGib( le );
				CG_AddRefEntityToScene( &le->refEntity, le->clientNum, le->refFlags, le->testPVS );

				VectorCopy( oldaxis[0], le->refEntity.axis[0] );
				VectorCopy( oldaxis[1], le->refEntity.axis[1] );
				VectorCopy( oldaxis[2], le->refEntity.axis[2] );
			}
			else {
//freeze
...
void CG_AddFadeRGB( localEntity_t *le ) {
	...
	CG_AddRefEntityToScene( re, le->clientNum, le->refFlags, le->testPVS ); //multiview
}
...
static void CG_AddMoveScaleFade( localEntity_t *le ) {
	...
	CG_AddRefEntityToScene( re, le->clientNum, le->refFlags, le->testPVS );
}
...
static void CG_AddExplosion( localEntity_t *ex ) {
	...
	if ( ent->renderfx & RF_THIRD_PERSON ) {
		CG_AddRefEntityToScene(ent, ex->clientNum, ex->refFlags, ex->testPVS);
	}
	else {
		CG_AddRefEntityToScene(ent, ex->clientNum, ex->refFlags, ex->testPVS);
	}
...
//			CG_StartSound( re->origin, ENTITYNUM_WORLD, CHAN_AUTO, cgs.media.kamikazeExplodeSound );
		...
		CG_AddRefEntityToScene( &shockwave, le->clientNum, le->refFlags, le->testPVS );
	}

	if (t > KAMI_EXPLODE_STARTTIME && t < KAMI_IMPLODE_ENDTIME) {
		// explosion and implosion
		c = ( le->endTime - time ) * le->lifeRate;
		c *= 0xff;
		re->shaderRGBA[0] = le->color[0] * c;
		re->shaderRGBA[1] = le->color[1] * c;
		re->shaderRGBA[2] = le->color[2] * c;
...
void CG_AddInvulnerabilityJuiced( localEntity_t *le ) {
	...
	if ( t > 5000 ) {
		le->endTime = 0;
		CG_GibPlayer( le->refEntity.origin, qfalse ); //freeze - added qboolean frozenBody
	}
	else {
		CG_AddRefEntityToScene( &le->refEntity, le->clientNum, le->refFlags, le->testPVS );
	}
