...
//freeze
	...
	if (!IsClientFrozen(ent) || !ent->target_ent) {
//freeze
...
//freeze
		...
		gentity_t *corpse = ent->target_ent;
		areanum = BotPointAreaNum(corpse->r.currentOrigin);
		if (areanum) {
			bs->teamgoal.entitynum = corpse->s.number;
			bs->teamgoal.areanum = areanum;
			VectorCopy(corpse->r.currentOrigin, bs->teamgoal.origin);
			VectorSet(bs->teamgoal.mins, -8, -8, -8);
			VectorSet(bs->teamgoal.maxs, 8, 8, 8);
			client = corpse->s.number;
		}
	}
//freeze
	//
