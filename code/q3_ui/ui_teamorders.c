...
static void UI_TeamOrdersMenu_BuildBotList( void ) {
	...
	int		clientNum = UI_Cvar_VariableIntegerValue( "cg_clientNum" );
	...
	playerTeam = BG_UnhashTeam( atoi(Info_ValueForKey(info, TEAM_KEY)), Info_ValueForKey(info, "n") );
	...
	for( n = 0; n < numPlayers && teamOrdersMenuInfo.numBots < 9; n++ ) {
		...
		botTeam = BG_UnhashTeam( atoi(Info_ValueForKey(info, TEAM_KEY)), Info_ValueForKey(info, "n") );
...
void UI_TeamOrdersMenu_f( void ) {
	...
	trap_GetClientState( &cs );
	trap_GetConfigString( CS_PLAYERS + clientNum, info, MAX_INFO_STRING );
	team = BG_UnhashTeam( atoi(Info_ValueForKey(info, TEAM_KEY)), Info_ValueForKey(info, "n") );
