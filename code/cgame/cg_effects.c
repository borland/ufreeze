...
//multiview
	le->clientNum = client;
	le->refFlags = REF_ONLY_CLIENT | REF_NOT_PIP;
//multiview
...
void CG_Bleed( vec3_t origin, int entityNum ) {
	...
	if ( entityNum == cg.snap->ps.clientNum ) {
		ex->clientNum = entityNum;
		ex->refFlags = REF_THIRD_PERSON;
	}
}
...
#define	GIB_JUMP		250
//freeze - added qboolean frozenBody
void CG_GibPlayer( vec3_t playerOrigin, qboolean frozenBody ) {
	vec3_t	origin, velocity;

//freeze
	if ( frozenBody ) {
		CG_IceChips( playerOrigin, 30 );
	}
//freeze
...
void CG_PortalEffect( vec3_t origin, vec3_t camera, vec3_t axis[3], qboolean swing, int rotateSpeed, int rollOffset ) {
	...
	VectorCopy( camera, ent.oldorigin );
	...
	ent.oldframe = swing;
	ent.frame = rotateSpeed;
	ent.skinNum = rollOffset;
