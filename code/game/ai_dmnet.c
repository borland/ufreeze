...
//freeze
	...
	if (bs->sneaky && BotVisibleEnemies(bs)) {
		bs->tfl &= ~TFL_JUMPPAD;
	}
//freeze
...
//freeze
	...
	if (bs->ltgtype == LTG_TEAMACCOMPANY && !bs->ordered) {
		trap_BotEmptyGoalStack(bs->gs);
		AIEnter_Seek_LTG(bs, "seek nbg: thawing");
		return qfalse;
	}
//freeze
	//
...
//freeze
	// if the bot is thawing
	if (bs->ltgtype == LTG_TEAMACCOMPANY && !bs->ordered) {
		vec3_t delta;
		gentity_t *teammate = &g_entities[bs->teammate];

		// if the teammate is valid
		if (teammate->inuse && teammate->target_ent) {
			VectorSubtract(bs->origin, teammate->r.currentOrigin, delta);

			// and close enough
			if (VectorLength(delta) < 60.0f) {
				// crouch
...
//freeze
	...
	//check for nearby goals periodically
	if (bs->check_time < FloatTime()) {
...
int AINode_Battle_Retreat(bot_state_t *bs) {
	...
	if (EntityIsInvisible(bs, &entinfo) > 0.9f) {
		if (random() < 0.2) {
			AIEnter_Seek_LTG(bs, "battle retreat: invisible");
			return qfalse;
		}
	}
	//if there is another better enemy
...
//freeze
	...
	if (bs->ltgtype == LTG_TEAMACCOMPANY && !bs->ordered) {
		trap_BotEmptyGoalStack(bs->gs);
		AIEnter_Seek_LTG(bs, "battle nbg: thawing");
		return qfalse;
	}
//freeze
	//
